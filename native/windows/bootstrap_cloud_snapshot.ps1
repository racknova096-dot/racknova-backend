...
function Ensure-SequencesFromCreateSql([string]$CreateSql) {
    if (-not $CreateSql) {
        return
    }
    ...
    foreach ($Match in $Matches) {
        ...
        $Parts = @($SequenceName.Split('.'))
        $CleanParts = @()

        foreach ($Part in $Parts) {
            $Clean = ([string]$Part).Trim('"')
            if ($Clean -notmatch '^[A-Za-z_][A-Za-z0-9_]*
    param(
        [Parameter(Mandatory=$true)]
        [string]$Table,
        [Parameter(Mandatory=$true)]
...
"@
    ...
    $Encoded = $null
                ...
                throw "Nombre de secuencia no permitido: $SequenceName"
            }
            $CleanParts += $Clean
        }

        if ($CleanParts.Count -eq 1) {
            $SequenceSql = 'CREATE SEQUENCE IF NOT EXISTS "' + $CleanParts[0] + '";'
        }
        elseif ($CleanParts.Count -eq 2) {
            $SequenceSql = (
            ...
            throw "Nombre de secuencia no soportado: $SequenceName"
        }

        Write-Log "Asegurando secuencia requerida: $SequenceName"
        Invoke-PsqlScript -Sql $SequenceSql | Out-Null
    }
}
