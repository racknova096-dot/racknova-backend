...
function Ensure-SequencesFromCreateSql([string]$CreateSql) {
    ...
    foreach ($Match in $Matches) {
        ...
        foreach ($Part in $Parts) {
            $Clean = ([string]$Part).Trim([char]34)
            $QuotedParts += (Quote-Ident $Clean)
        }

        if (($QuotedParts.Count -lt 1) -or ($QuotedParts.Count -gt 2)) {
            throw "Nombre de secuencia no soportado: $SequenceName"
        }

        $SequenceSql = "CREATE SEQUENCE IF NOT EXISTS " + ($QuotedParts -join ".") + ";"
